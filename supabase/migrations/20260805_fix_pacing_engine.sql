-- Migration: 20260805_fix_pacing_engine.sql
-- Description: Fix and repair pre-computed pacing engine function to assign distinct randomized incremental timestamps incorporating random delay jitter
...
-- RE-RUN IMMEDIATELY FOR ALL ACCOUNTS TO RE-CALCULATE ALL PENDING PINS WITH RANDOM DELAYS
DO $$ 
DECLARE 
  acc_id UUID; 
BEGIN 
  FOR acc_id IN SELECT id FROM public.accounts LOOP 
    PERFORM public.reschedule_account_pending_pins(acc_id); 
  END LOOP; 
END $$;
