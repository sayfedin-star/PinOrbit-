...
export async function assertWorkspaceAccess(
  schedulingClient: SupabaseClient, workspaceId: string, userId: string, requiredRole: RequiredRole = 'member'
): Promise<WorkspaceContext> {
  if (!workspaceId || !userId) throw new HttpError(401, 'Unauthorized: missing workspace or user identifier.');
